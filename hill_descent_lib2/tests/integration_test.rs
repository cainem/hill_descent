//! Integration test for hill_descent_lib2.
//!
...
fn given_world_when_complete_optimization_workflow_then_succeeds() {
    ...
    // Score should not get worse with more training
    let score_after_training = final_score;
    world.training_run(TrainingData::None { floor_value: 0.0 });
    assert!(
        world.get_best_score() <= score_after_training,
        "Score should not regress"
    );
}
