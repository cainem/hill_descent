...
fn given_fixed_seed_when_training_run_in_single_thread_pool_then_best_is_golden() {
    // Updated golden values after excluding dead-from-age organisms from reproduction
    const EXPECTED_BEST_SCORE: f64 = 5.500657214527858e-7;
    const EXPECTED_BEST_PARAMS: [f64; 2] = [-0.0005034952382109159, 0.000544571635831062];
    const EXPECTED_BEST_ID: Option<u64> = Some(5695);
    ...
    pool.install(|| {
        ...
        // Force the messaging_thread_pool organism pool to 1 thread as well.
        // Otherwise, even with Rayon configured to 1 thread, lib2 can still be nondeterministic.
        let mut world = World::new_with_thread_count(&bounds, constants, Box::new(Sphere), 1);
