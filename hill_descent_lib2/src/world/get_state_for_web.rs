...
use super::World;
use crate::organism::{GetWebStateRequest, GetWebStateResponse};
use serde::Serialize;
...
impl World {
    ...
    pub fn get_state_for_web(&self) -> String {
        ...
        // Query all organisms for their web state
        let requests = self.organism_ids.iter().map(|&id| GetWebStateRequest(id));

        let responses: Vec<GetWebStateResponse> = self
            .organism_pool
        ...
        let organisms: Vec<OrganismState> = responses
            .into_iter()
            .filter_map(|response| {
