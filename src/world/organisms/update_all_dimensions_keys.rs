...
impl Organisms {
    /// Updates the dimensions key for all phenotypes.
    ///
    /// Iterates through each phenotype and calls `update_dimensions_key`.
    ...
    ///
    /// # Returns
    /// * `Ok(())` if all phenotype dimension keys were updated successfully.
...
mod tests {
    ...
    #[test]
    fn given_no_phenotypes_when_update_all_dimensions_keys_then_ok() {
        let mut organisms = Organisms::new_from_phenotypes(vec![]);
        let dimensions = create_test_dimensions_for_organisms(2, 4);
        assert_eq!(organisms.update_all_dimensions_keys(&dimensions), Ok(()));
    }

    #[test]
    fn given_phenotypes_all_update_successfully_when_update_all_dimensions_keys_then_ok() {
    ...
    #[test]
    fn given_one_phenotype_fails_update_when_update_all_dimensions_keys_then_err() {
        let mut rng = thread_rng();
        ...
        let mut failing_p = Phenotype::new_random_phenotype(&mut rng, &full_bounds);

        // To make failing_p fail, we create dimensions where its naturally-expressed value is out of bounds.
        ...
        // Pre-check: ensure our setup is correct and failing_p does fail.
        assert!(
            failing_p
                .update_dimensions_key(&dimensions_that_cause_failure)
                .is_err()
        );

        test_phenotypes.push(failing_p); // Add the failing phenotype.
        ...
        let result = organisms.update_all_dimensions_keys(&dimensions_that_cause_failure);
