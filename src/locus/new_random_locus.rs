...
impl Locus {
    ...
    pub fn new_random(rng: &mut impl Rng, value_bounds: &RangeInclusive<f64>) -> Self {
        ...
        let locus_value_param = Parameter::with_bounds(val, *value_bounds.start(), *value_bounds.end());
        ...
        let mut adjustment = LocusAdjustment::new_random(rng, value_bounds);
        ...
        Self::new( // Use the public constructor of Locus (Self::new)
            locus_value_param,
            adjustment,
...
mod tests {
    ...
    fn given_rng_and_bounds_when_new_random_then_locus_is_valid() {
        ...
        for _ in 0..100 {
            let locus = Locus::new_random(&mut rng, &bounds); // Changed call
            assert!(
    ...
    fn given_new_random_locus_when_dh_flag_true_and_doubling_out_of_bounds_then_dh_flag_is_false() {
        ...
        for i in 0..10000 { // Increased attempts to find the specific scenario
            ...
            let initial_adjustment = LocusAdjustment::new_random(&mut adj_rng, &bounds); // Changed call

            if initial_adjustment.doubling_or_halving_flag() && initial_adjustment.direction_of_travel() == DirectionOfTravel::Add {
                ...
                if !bounds.contains(&hypothetical_doubled_locus_val) {
                    ...
                    let locus = Locus::new_random(&mut final_rng, &bounds); // Changed call
                    let final_dh_flag = locus.adjustment.doubling_or_halving_flag();
                    assert!(!final_dh_flag, "D/H flag should be false. Seed {}, Locus val: {}, Adj direction: {:?}, Initial D/H would have been true", i, locus_candidate_val, initial_adjustment.direction_of_travel());
                    found_case = true;
                    break;
                }
            }
        }
        assert!(found_case, "Test case for doubling out of bounds not found. Adjust seed search or logic.");
    }
    ...
    fn given_new_random_locus_when_dh_flag_true_and_halving_out_of_bounds_then_dh_flag_is_false() {
        ...
        for i in 0..10000 {
            ...
            if initial_adjustment.doubling_or_halving_flag() && initial_adjustment.direction_of_travel() == DirectionOfTravel::Subtract {
                let hypothetical_halved_locus_val = locus_candidate_val / 2.0;
                if !bounds.contains(&hypothetical_halved_locus_val) {
                    let mut final_rng = StdRng::seed_from_u64(i);
                    let locus = Locus::new_random(&mut final_rng, &bounds); // Changed call
                    let final_dh_flag = locus.adjustment.doubling_or_halving_flag();
                    assert!(!final_dh_flag, "D/H flag should be false for halving out of bounds. Seed {}, Locus val: {}, Adj direction: {:?}, Initial D/H would have been true", i, locus_candidate_val, initial_adjustment.direction_of_travel());
                    found_case = true;
                    break;
                }
            }
        }
        assert!(found_case, "Test case for halving out of bounds not found. Adjust seed search or logic.");
    }
        ...
        for i in 0..10000 {
            ...
            if initial_adjustment.doubling_or_halving_flag() {
                ...
                if bounds.contains(&hypothetical_val) {
                    ...
                    let locus = Locus::new_random(&mut final_rng, &bounds); // Changed call
                    let final_dh_flag = locus.adjustment.doubling_or_halving_flag();
                    assert!(final_dh_flag, "D/H flag should be true. Seed {}, Locus val: {}, Adj direction: {:?}, Hypothetical: {}", i, locus_candidate_val, initial_adjustment.direction_of_travel(), hypothetical_val);
                    found_case = true;
                    break;
                }
            }
        }
        assert!(found_case, "Test case for D/H flag true and in bounds not found.");
    }
    ...
    fn given_new_random_locus_when_initial_dh_flag_false_then_final_dh_flag_is_false() {
        ...
        assert!(found_case, "Test case for initial D/H flag false not found.");
    }
}
