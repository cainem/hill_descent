...
use crate::gen_hybrid_range::gen_hybrid_range;
...
impl Locus {
    ...
    pub fn new_random_locus(rng: &mut impl Rng, value_bounds: &RangeInclusive<f64>) -> Locus {
        let mut locus_value;
        let mut locus_adjustment;

        // This loop ensures that we don't get stuck in a state where the generated
        // value is at a boundary and the adjustment direction would immediately push it
        // out of bounds, causing an infinite loop.
        loop {
            locus_value = gen_hybrid_range(rng, value_bounds.clone());
            locus_adjustment = LocusAdjustment::new_random_locus_adjustment(rng, value_bounds);

            if locus_adjustment.doubling_or_halving_flag() {
                let prospective_val = match locus_adjustment.direction_of_travel() {
                    DirectionOfTravel::Add => locus_value * 2.0,
                    DirectionOfTravel::Subtract => locus_value / 2.0,
                };

                if value_bounds.contains(&prospective_val) {
                    break; // This combination is safe, exit the loop.
                }
                // Otherwise, the loop continues, generating a new value and adjustment.
            } else {
                break; // No doubling/halving, so it's safe by default.
            }
        }

        let locus_value_param =
            Parameter::with_bounds(locus_value, *value_bounds.start(), *value_bounds.end());
